...
def main() -> int:
    ...
    print(f"{'model':<24} {'runs':>4} {'pass':>5} {'pass%':>6} "
    ...
    for model, runs in sorted(by_model.items()):
        ...
        tps = [r["metrics"]["tokens_per_s"] for r in runs
               if r.get("metrics", {}).get("tokens_per_s") is not None]
        ttft = [r["metrics"]["ttft_ms"] for r in runs
                if r.get("metrics", {}).get("ttft_ms") is not None]
        wall = [r["metrics"]["wall_s"] for r in runs
                if r.get("metrics", {}).get("wall_s") is not None]
        print(f"{model:<24} {len(runs):>4} {passes:>5} "
