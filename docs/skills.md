...
`wiki-pii-scan` keeps its allowlist **repo-local** (`<repo-root>/etc/pii-allowlist`), not in agent-lab, because the allowlist content (e.g. `@sipgate.de`, service accounts) is organization-specific. The helper is generic; the data is not. A repo without an allowlist file runs strict (over-blocks rather than under-blocks).
